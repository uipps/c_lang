...
int Magic3(int **magic, int n)
{
    ...
    for (i = 0; i < n; i++) //第一步,按顺序填入数字
        for (j = 0; j < n; j++)
            magic[i][j] = i * n + j + 1;

    for (i = sn + 1; i <= 3 * sn; i++) //第二步，将井字左右部分进行翻转
    {
        for (j = 0; j <= n / 2 - 1; j++)
        {
            if (j != sn)
    ...
    for (i = sn + 1; i <= 3 * sn; i++) //第二步，井字上下部分进行翻转
        for (j = 0; j <= n / 2 - 1; j++)
            if (j != sn)
                swap(&magic[j][i], &magic[n - 1 - j][i]);

    for (i = 0; i <= sn - 1; i++) //第三步，将井字分割线之横列及第k+2列两侧数字对调
    {
    ...
    for (i = 0; i <= n / 2 - 1; i++) //左变列除交点外的数字垂直翻转
        if (i != sn)
            swap(&magic[i][sn], &magic[n - 1 - i][sn]);

    swap(&magic[sn][0], &magic[n - 1 - sn][0]); //第四步，将井字横线左侧数字上下对调

    for (i = sn + 1; i <= n / 2 - 1; i++) //井字横线中央翻转
        swap(&magic[sn][i], &magic[sn][n - 1 - i]);
    for (i = sn + 1; i <= n - 1 - sn - 1; i++) //井字分割线纵向中央数字除第2K+1列外左右对调
        if (i != n / 2 - 1)
