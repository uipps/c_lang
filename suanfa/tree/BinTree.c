...
ChainBinTree *BinTreeFind(ChainBinTree *bt, DATA data) //在二叉树种查找值为data的结点
{
    ChainBinTree *p;
    if (bt == NULL)
        return NULL;
...
void BinTreeClear(ChainBinTree *bt) //清空二叉树，使之变为一颗空树
{
    if (bt)
    {
...
void BinTree_LDR(ChainBinTree *bt, void (*oper)(ChainBinTree *p)) //先序遍历
{
    if (bt) //树不为空，则执行如下操作
    {
...
void BinTree_LRD(ChainBinTree *bt, void (*oper)(ChainBinTree *p)) //先序遍历
{
    if (bt) //树不为空，则执行如下操作
    {
...
void BinTree_Level(ChainBinTree *bt, void (*oper)(ChainBinTree *p)) //按层遍历
{
    ...
    int head = 0, tail = 0;         //队首、对尾序号
    if (bt)                         //若队首指针不为空
    {
        tail = (tail + 1) % QUEUE_MAXSIZE; //计算循环队尾序号
        q[tail] = bt;                      //将二叉树指针进队
    }
    while (head != tail) //队列不为空，进行循环
    {
        head = (head + 1) % QUEUE_MAXSIZE; //计算循环对列队尾序号
        p = q[head];                       //获取队首元素
        oper(p);                           //处理队首元素
        ...
        if (p->right != NULL) //若结点存在右子树，则右孩子结点指针进队
        {
            tail = (tail + 1) % QUEUE_MAXSIZE; //计算循环队列的队尾序号
