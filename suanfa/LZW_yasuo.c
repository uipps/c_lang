// LZW_yasuo.exe -z README.md readme.zip
// LZW_yasuo.exe -e "readme.zip" readme2.md.txt
...
void compress(FILE *input, FILE *output) //压缩函数
{
    ...
    if (!(lzw1.prefix = malloc(TABLE_SIZE * sizeof(unsigned int)))) //压缩前函数
    {
        printf("内存分配失败\n");
        exit(0);
    }
    ...
    lzw = &lzw1;
    curr_code = 258;                 //编码表中的字符串编码从258开始
    for (i = 0; i < TABLE_SIZE; i++) //初始化标号开始
        lzw->code[i] = -1;
    i = 0;
    printf("\n开始压缩");
...
void expand(FILE *input, FILE *output) //解压缩函数
{
    ...
    while ((suffix = incode(input)) != (MAX_VALUE)) //循环进入解压缩
    {
        if (++i == 1000) //处理1000个字符就显示一个点
        {
            i = 0;
            printf(".");
        }
        ...
        while (ps >= decode_stack) //循环输出解码的字节
            putc(*ps--, output);
        if (curr_code <= MAX_CODE) //若标记未超过最大值
        {
...
char *decode(unsigned char *buffer, unsigned int code) //编码函数
{
    int i = 0;
    ...
    *buffer = code;  //将标号放入缓存区
    return (buffer); //返回缓存区中的内容
}
unsigned int incode(FILE *input) //从压缩文件中读取数据
{
    unsigned int ret;
    static int ib = 0;             //静态变量，保存读入数据的二进制位数
    static unsigned long ibb = 0L; //静态变量，保存已读入数据的二进位
    while (ib <= 24)               //若数据位小于24位
    {
        ibb |= (unsigned long)getc(input) << (24 - ib);
        //从文件中获取一个字节，并组合到需输出的二进制位中
        ib += 8;
    }
...
int main(int argc, char *argv[])
{
    ...
    int flag = 0;
    char *op;
    if (argc != 4) //判断传入 参数的数量
        flag = 1;
    else
    {
        op = argv[1];
        if ((strcmp(op, "-z") != 0) && strcmp(op, "-e") != 0)
            flag = 1;
    }
    if (flag)
    {
    ...
    if ((fp2 = fopen(argv[3], "wb")) == NULL) //打开文件出错
    {
        printf("不能创建目标文件!\n");
        exit(1);
    }
